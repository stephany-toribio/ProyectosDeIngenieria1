...
void onColorRgbChange() {
  // Do something when color_rgb changes
  uint8_t r, g, b;
  color_rgb.getValue().getRGB(r, g, b);
  if (color_rgb.getSwitch()) {
...
void onRelay1Change() {
  // Do something when relay_1 changes
  if (relay_1) {
    carrier.Relay1.open();
    relayState1 = "ON";
  } else {
...
void onRelay2Change() {
  // Do something when relay_2 changes
  if (relay_2) {
    carrier.Relay2.open();
    relayState2 = "ON";
  } else {
